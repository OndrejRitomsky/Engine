...
namespace eng {
	...
	void PlatformModule::UpdateWindowEvents() {
		...
		while (eventsBuffer < end) {
			...
			{
				core::PlatformWindowCloseEvent* resize = (core::PlatformWindowCloseEvent*) eventsBuffer;
				_closeRequested = true;
				eventsBuffer += sizeof(core::PlatformWindowCloseEvent);
				break;
			}
			case core::PLATFORM_EVENT_WINDOW_RESIZE:
			{
				core::PlatformWindowResizeEvent* resize = (core::PlatformWindowResizeEvent*) eventsBuffer;
				_windowWidth = resize->newWidth;
				_windowHeight = resize->newHeight;
				_windowSizeChanged = true;
				eventsBuffer += sizeof(core::PlatformWindowResizeEvent);
				break;
			}
			default:
				ASSERT(false);
