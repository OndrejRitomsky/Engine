...
namespace core {
	static_assert(alignof(PlatformWindowResizeEvent) == 8, "Alignment missmatch");
	static_assert(alignof(PlatformWindowCloseEvent) == 8, "Alignment missmatch");
	...
	LRESULT CALLBACK W32_WindowCallback(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam) {
		...
		if (!window) {
			...
			case WM_SIZE:
			{
				...
				PlatformWindowResizeEvent resizeEvent = {PLATFORM_EVENT_WINDOW_RESIZE, size.width, size.height};
				window->eventsBuffer.Append((char*) &resizeEvent, sizeof(resizeEvent));
			}
			break;
			case WM_DESTROY:
			{
				PlatformWindowCloseEvent closeEvent = {PLATFORM_EVENT_CLOSE};
				window->eventsBuffer.Append((char*) &closeEvent, sizeof(closeEvent));
			}
			break;
			case WM_CLOSE:
			{
