...
namespace eng {
	...
	bool CTransform2DComponentManager::Remove(Entity* entity) {
		...
		if (i < last) {
			Transform2DComponent* lastHandle = _map.Find(_data.id[last].h);
			ASSERT(lastHandle);
	...
	const V2&  CTransform2DComponentManager::Position(Transform2DComponent handle) const {
		return _data.position[handle.h];
	}

	void CTransform2DComponentManager::SetPosition(Transform2DComponent handle, const V2&position) {
		_data.position[handle.h] = position;
		_data.dirty[handle.h] = true;
	}

	const V2& CTransform2DComponentManager::Scale(Transform2DComponent handle) const {
		return _data.scale[handle.h];
	}

	void CTransform2DComponentManager::SetScale(Transform2DComponent handle, const V2& scale) {
		_data.scale[handle.h] = scale;
		_data.dirty[handle.h] = true;
	}

	const M44& CTransform2DComponentManager::Transform(Transform2DComponent handle) const {
